...
first, we laod all data and we perform data cleaning and quality controls.
...
source("../R/Evaluate_model_function.R") #This function takes many arguments, these will be elaborated step by step.
  ...
  temp_lm<- model_daily_fluxes(daily_fluxes,method_loc = "lm",split_prop = 0.7) #returns the model with "lm" and a split proportion of 0.7, as well as the datasets of the splits. The return is a named list.
  print(eval_model(mod = temp_lm$mod, df_train = temp_lm$daily_fluxes_train, df_test = temp_lm$daily_fluxes_test)) #The function creates an evaluation of the model based on the given datasets of training and evaluation. Naming can be specified with parameters as well as other outputs, these will be used later on.
  
rm(temp_lm) #to keep workspace tidy
...
```{R}
# KNN
  temp_KNN <- model_daily_fluxes(daily_fluxes,"knn",kofKNN = 8,split_prop = 0.7) #the same method is used, but with knn and a k of 8
  print(eval_model(mod = temp_KNN$mod, df_train = temp_KNN$daily_fluxes_train, df_test = temp_KNN$daily_fluxes_test)) 
  rm(temp_KNN)
...
Because regression predicts what the connections there are between variables for all variables (global) and so for both datasets behaves similarly because there is a lot of data in the training part that is all fitted with one line and so the regression line will miss also points of the training data (maybe underfit), while nearest neighbour calculates a local regression so it fits training data very well (somehow maybe overfitting?) but may not fit new data as well. Here this is not the case and the knn model appreas to outperform the lm model even in the test dataset and so is probably the better fit. This is probably because the knn model captures local trends better due to the aforementioned factors.
...
```{R}
  ...
  print(eval_model(mod = temp_KNN$mod, df_train = temp_KNN$daily_fluxes_train, df_test = temp_KNN$daily_fluxes_test, out = "plot_all")) #here we now also want additional plots which show the distribution over time so the output is set to "all plots"
...
Not much variation in the residuals (fitted-actual value, on the y axis) over time, residuals were used to show the variations of observed and modeled GPP. 
...
rm(k)
```
The aforementioned things are visible in these plots: with K = 1 the training dataset performes very well but the test dataset performs miserable. 
As of for the other extreme, the model also appreas to slightly decrease the R2 value as well as increase the MAE when the k becomes extremely high. This is due to underfitting with to many neighbors chosen.


## Is there an “optimal”  k in terms of model generalisability? Edit your code to determine an optimal k
For the "optimal" i would argue based on my current knowledge that we want the best performance possible in the R2 in the test dataset.
We can loop over several possibilities of k and then do an evaluation:

 
```{R}
rsq_test <-  NULL #Vector to save the R2 values
values_k <- c(1,seq(5,to = 100, by = 5),125,150) #values to be tested
for (k in values_k) {
  
  temp_KNN <- model_daily_fluxes(daily_fluxes,"knn",k,0.7) #create the model
...
rsq_test <- tibble(k = values_k,rsq_test = rsq_test) #save values as a nice tibble


ggplot(data = rsq_test,aes(x = k, y = rsq_test))+
  geom_point()+geom_line()+theme_classic()+
  labs(x = "k of KNN",y = "R^2")#plot the data

rsq_test |>
  filter(rsq_test == max(rsq_test)) #print the best performing model.
rm(temp_KNN,rsq_test,values_k,k)

```
As predicted in our less elaborate approach, the R2 value of the test data increases at first (with some ranom fluctuations) until k = 60 when it starts to decrease again. Therefor k = 60 is our "optimal" k.

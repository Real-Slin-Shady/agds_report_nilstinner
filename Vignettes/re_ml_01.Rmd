...
For the "optimal" i would argue based on my current knowledge that we want the best performance possible in the RMSE in the test dataset.
We can loop over several possibilities of k and then do an evaluation:

 
```{R}
rmse_test <-  NULL #Vector to save the R2 values
values_k <- c(1,seq(5,to = 100, by = 5),125,150) #values to be tested
for (k in values_k) {
  
    temp_KNN <- model_daily_fluxes(split$train,split$test,method_loc = "knn",kofKNN = k)
  rmse_test <- c(rmse_test, eval_model(mod = temp_KNN, df_train = split$train, df_test = split$test, out = "rmse")$rmse_test)
...
rmse_test <- tibble(k = values_k,rmse_test = rmse_test) #save values as a nice tibble


ggplot(data = rmse_test,aes(x = k, y = rmse_test))+
  geom_point()+geom_line()+theme_classic()+
  labs(x = "k of KNN",y = "RMSE")#plot the data

rmse_test |>
  filter(rmse_test == min(rmse_test)) #print the best performing model.
rm(temp_KNN,rmse_test,values_k,k)

```
As predicted in our less elaborate approach, the RMSE value of the test data increases at first (with some ranom fluctuations) until k = 60 when it starts to decrease again. Therefor k = 60 is our "optimal" k.

...
daily_fluxes_dav_test_plot <- melt(daily_fluxes_dav_test_plot, id.vars = "TIMESTAMP", value.name  = "GPP_NT_VUT_REF")


  ggplot(daily_fluxes_dav_test_plot,aes(x = TIMESTAMP,y = GPP_NT_VUT_REF,col = Year))+
    geom_line()


```

The model preforms better for davos than laegern. This might be because davos just simply has more datapoints and therefor is weighted heavier than laegern which introduces a bias towards more accurate predictions for davos.


#idea: deciduous vs broadleave: shouldt there be a winter bias? No? -> dont understand last part of questions...
#Mabye bias? -> where model tends to?